# here is the transformation part of our pipeline
...
class ETLTransformationError(Exception):
    pass
...
def transform_data(customers, orders, payments):
    ...
    try:
        ...
        raise ETLTransformationError(
            f"Transformation failed: required column is missing - {e}"
        ) from e

    except (ValueError, TypeError) as e:
        raise ETLTransformationError(
            f"Transformation failed: invalid data or data type - {e}"
        ) from e

    except Exception as e:
        raise ETLTransformationError(
            f"Transformation failed due to an unexpected error: {e}"
        ) from e
